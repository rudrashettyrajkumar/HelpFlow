...
def _env(monkeypatch):
    ...
    # honoured (importing litellm elsewhere can run load_dotenv()).
...
def assert_no_llm_calls():
    ...
    the router's public calls (`complete`/`stream`) — what agents use. The stubs
    return inert values instead of raising so a swallowing try/except can't hide
    ...
    with (
        patch.object(llm_router, "complete", _async),
        patch.object(llm_router, "stream", _agen),
    ):
        yield guard
